...
Player::Player(const CL_String& name, const CL_String& password, bool login) : name(name), password(password)
{
    try 
    {
        ...
        if (login) 
        {
            CL_DBCommand login_cmd = connection.create_command("SELECT PlayerId, Progress FROM Players WHERE PlayerName=?1 AND Password=?2", name, password);
        ...
        else 
        {
            ...
            CL_DBCommand create_cmd = connection.create_command("INSERT OR IGNORE INTO Players (PlayerName, Password) VALUES (?1, ?2)", name, password);
            ...
            CL_DBCommand id_cmd = connection.create_command("SELECT PlayerId FROM Players WHERE PlayerName=?1", name);
...
int Player::getLevelScore(int level_id)
{
    ...
    CL_DBCommand get_score_cmd = connection.create_command("SELECT Score FROM Scores WHERE PlayerId=?1 AND LevelId=?2", id, level_id);
...
void Player::updateProgress(int newlevel)
{
    ...
    CL_DBCommand update_cmd = connection.create_command("UPDATE Players SET Progress=?1 WHERE PlayerId=?2", newlevel, id);
...
void Player::updateLevelScore(int level_id, int newscore) 
{
    try 
    {
        ...
        CL_DBCommand count_cmd = connection.create_command("SELECT COUNT(*) FROM Scores WHERE LevelId=?1 AND PlayerId=?2", level_id, id);
        int retval = connection.execute_scalar_int(count_cmd);

        // If the score for that player and level exists.
        if (retval) 
        {
            CL_DBCommand update_cmd = connection.create_command("UPDATE Scores SET Score=?1 WHERE LevelId=?2 AND PlayerId=?3", newscore, level_id, id);
            connection.execute_non_query(update_cmd);
        }
        // If the score for that player and level doesn't exist.
        else
        {
            CL_DBCommand create_cmd = connection.create_command("INSERT INTO Scores VALUES (?1,?2,?3)", level_id, id, newscore);
            connection.execute_non_query(create_cmd);
        }
    }
    catch (CL_Exception& e) 
...
void Player::deletePlayer()
{
    try 
    {
        ...
        CL_DBCommand del_player_cmd = connection.create_command("DELETE FROM Players WHERE PlayerId=?1", id);
        CL_DBCommand del_score_cmd = connection.create_command("DELETE FROM Scores WHERE PlayerId=?1", id);
...
// This function prints both the Players and the Scores database. It also prints the information of the player given as param.
// This function is mainly for debugging and testing purposes.
